...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    self.documentsDir = paths[0];
    NSString *inputFilePath = [self.documentsDir stringByAppendingPathComponent:@"input.txt"];    
    NSError  *err;
    

    phaseNames = @[@"Memorize", @"Verify", @"Enter", @"Recall"];
    phaseShortNames = @[@"M", @"V", @"E", @"R"];
    ...
    if ([fileManager fileExistsAtPath:inputFilePath] == NO ) {
        ...
        NSArray *array = @[@"ABCD1", @"ABCD2", @"ABCD3",@"ABCD4",@"ABCD5",@"ABCD6",@"ABCD7",@"ABCD8",@"ABCD9",@"ABCD10"];
...
-(void) initRolledUpData {
    ...
    for ( int i=0; i < 10; i++ ) {
        ...
        data->targetString = [[NSString alloc] initWithString:inputCharStrings[i]];
...
-(void) userBeginsTest {
    ...
    if ([fileManager fileExistsAtPath:rawFilePath] == NO ) {
        // create file default file
        NSArray *array = @[@"MilliSeconds", @"Phase or Participant", @"String Number", @"Phase Code", @"Character Code", @"Entered String", @"Target String", @"Match", @"X-Cord", @"Y-Cord" @"\n"];
    ...
    if ([fileManager fileExistsAtPath:rolledFilePath] == NO ) {
        // create file default file
        NSArray *array = @[@"Participant Id", @"Target String", @"Entered String", @"Match", @"Elapse Time", @"String Number", @"Phase Code", @"\n"];
...
-(void) insertIndividualMemoryTime: (NSTimeInterval) time andString:str forStringNum: (int) index {
    ...
    NISTPerStringRolledUpData *temp = rolledUpDataPerString[index];
    
    // add individual memorize time
    [temp->individualMemorizeTime addObject: @(time)]; 
...
-(void) insertIndividualVerifyTime: (NSTimeInterval) time andString:str forStringNum: (int) index {
    ...
    [temp->individualVerifyTime addObject: @(time)]; 
...
-(NSString *) didMatchAnyInputString: (NSString *) s {
    ...
    for (int i=0; i < count; i++) {
        if ( [inputCharStrings[i] isEqualToString: s] ) {
            match = @"True";
            break;
        }
    }
...
-(void) logRolledUpDataForString: (int) index {
    ...
    // Get PerStringData for string number index
    NISTPerStringRolledUpData *data = rolledUpDataPerString[index];
    ...
    for (NSNumber *n in data->individualMemorizeTime) {
        ...
        NSString    *phaseStr;
        phaseStr = [[NSString alloc] initWithFormat:@"%@%i", phaseShortNames[NISTMemorize], i];

        enteredString = data->individualVerifyStrings[i-1]; 
    ...
    for (NSNumber *n in data->individualVerifyTime) {
        ...
        NSString    *phaseStr;
        phaseStr = [[NSString alloc] initWithFormat:@"%@%i", phaseShortNames[NISTVerify], i];
    ...
    NSString    *phaseStr;
    phaseStr = [[NSString alloc] initWithFormat:@"%@T", phaseShortNames[NISTMemorize]];
    NSString *record;
    ...
    sec = (long) (time * 1000);    
    phaseStr = [[NSString alloc] initWithFormat:@"%@T", phaseShortNames[NISTVerify]];
    record = [[NSString alloc] initWithFormat:@"%@,%@,%@,%@,%ld,%i,%@\n", participantId, data->targetString, notApplicable, notApplicable, sec, index+1, phaseStr ];
    [self logToRolledUpFile:record];
    ...
    for (int k=0; k < 10; k++ ) {
        ...
        NSString    *phaseStr;
        phaseStr = [[NSString alloc] initWithFormat:@"%@%i", phaseShortNames[NISTEnter], k+1];
        enteredString = data->individualEntrySrings[k];
        if ( [data->targetString isEqualToString:enteredString] )
            match = @"True";
        else 
            match = @"False";
    ...
    phaseStr = [[NSString alloc] initWithFormat:@"%@T", phaseShortNames[NISTEnter]];
    record = [[NSString alloc] initWithFormat:@"%@,%@,%@,%@,%ld,%i,%@\n", participantId, data->targetString, notApplicable, notApplicable, sec, index+1, phaseStr ];
    [self logToRolledUpFile:record];
...
-(void) logRecalledAndSessionData {
    ...
    long sec = (long) (time * 1000);
    NSString    *phaseStr;
    phaseStr = [[NSString alloc] initWithFormat:@"%@TT", phaseShortNames[NISTEnter]];
    NSString *record;
    ...
    for (NSString *r in recalledStrings) {       
        NSString    *phaseStr;
        phaseStr = [[NSString alloc] initWithFormat:@"%@%i", phaseShortNames[NISTRecall], i];
        NSString *record;
        
        NSString *match = recalledMatched[i-1];
    ...
    sec = (long) (time * 1000);
    phaseStr = [[NSString alloc] initWithFormat:@"%@T", phaseShortNames[NISTRecall]];
    record = [[NSString alloc] initWithFormat:@"%@,%@,%@,%@,%ld,%@,%@\n", participantId, notApplicable, notApplicable, notApplicable, sec, notApplicable, phaseStr ];
    [self logToRolledUpFile:record];
...
- (NSString *) getCurrentString {
    ...
    if ( inputCharStrings != nil )
        str = inputCharStrings[currentStringNumber];
...
-(NSString *) getRawDataRecord: (NISTUserEvent *) event {
    ...
    if ( event.eventType == NISTEventTypeInput ) {
        ...
        NSString    *phaseStr;
        if ( event.subPhase != 0 )
            phaseStr = [[NSString alloc] initWithFormat:@"%@%i", phaseShortNames[index], event.subPhase];
        else {
            phaseStr = [[NSString alloc] initWithFormat:@"%@", phaseShortNames[index]];
        }
